...
function SequenceTimingDiagram() {
  ...
  const innerW = W - PAD.l - PAD.r
  const ROW_H = 16  // height per waveform row
  const ROW_Y = [6, 22, 38, 54, 70, 86]  // y baselines: RF, Gss, Gpe, Gro, ADC, labels
  const AMP = ROW_H * 0.82  // waveform amplitude
  const MID = (y: number) => ROW_Y[y] + ROW_H * 0.5  // midline of each row

  // Display TR capped at 3000ms for readability
  ...
  // Time points
  const t_exc = isIR ? params.TI : 0                // excitation after TI
  const t_refocus = t_exc + params.TE / 2           // 180° refocus (SE/TSE)
  const t_echo = t_exc + params.TE                  // first echo center
  const t_inversion = 0                             // IR inversion pulse

  // Sinc RF pulse shape (simplified, 7 points)
  ...
  // Rectangular pulse (for 180° refocus / hard pulse)
  ...
  return (
    <div className="mx-3 mt-2 p-1.5 rounded" style={{ background: '#050709', border: '1px solid #111d28' }}>
      ...
      <svg width={W} height={H} style={{ display: 'block', overflow: 'visible' }}>
        ...
        {/* ──── Gpe Row ──── */}
        {/* Phase encode blip before each echo (varying amp = dashed hint) */}
        ...
        {/* Readout gradients for each echo */}
