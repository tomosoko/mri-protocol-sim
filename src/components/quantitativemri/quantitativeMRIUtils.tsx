...
export function T1MappingVFA({ fieldStrength }: { fieldStrength: number }) {
...
export function T2MappingMSE({ fieldStrength }: { fieldStrength: number }) {
  const is3T = fieldStrength >= 2.5
...
export function T2StarMappingGRE({ fieldStrength }: { fieldStrength: number }) {
  const is3T = fieldStrength >= 2.5
...
// ── 磁化率強調像 (SWI) フィルタ説明 ──────────────────────────────────────────
export function SWIExplainer({ fieldStrength }: { fieldStrength: number }) {
  const is3T = fieldStrength >= 2.5
...
export function MRSExplainer({ fieldStrength }: { fieldStrength: number }) {
  const is3T = fieldStrength >= 2.5
  const peaks = [
...
// ── 定量MRI まとめテーブル (常時表示) ────────────────────────────────────────
export function QMRISummaryTable() {
...
export type QSubTab = 'T1map' | 'T2map' | 'SWI' | 'MRS'

export const subTabStyle = (active: boolean) => ({
